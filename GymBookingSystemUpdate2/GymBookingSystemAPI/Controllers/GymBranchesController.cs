    ...
    public class GymBranchesController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<GymBranchDetailDTO>>> GetGymBranches()
        {
            var branches = await _context.GymBranches
                ...
                .ToListAsync(); // Removed the .Where(b => b.IsActive) to show all branches
        ...
        public async Task<ActionResult<GymBranchDetailDTO>> GetGymBranchDetail(int id)
        {
            ...
            {
                return NotFound(new { message = "Chi nhánh không tồn tại hoặc đã bị tắt." });
            }
        ...
        // PUT: api/GymBranches/5/ToggleStatus
        [HttpPut("{id}/ToggleStatus")]
        public async Task<IActionResult> ToggleBranchStatus(int id, [FromBody] bool isActive)
        {
            var branch = await _context.GymBranches.FindAsync(id);
            if (branch == null)
            {
            ...
            {
                return StatusCode(500, new { message = "Có lỗi xảy ra khi cập nhật trạng thái chi nhánh: " + ex.Message });
            }
        }
